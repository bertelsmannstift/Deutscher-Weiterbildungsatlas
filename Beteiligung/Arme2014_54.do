...
*** Datengrundlage: 				<Mikrozensus 2014>
***
*** Dateiname des Programmcodes: 	<Arm2014.do>
*** erstellt: 						<013.06-2018> 
*** von: 							<Andreas Martin> 
*** E-Mail: 						<martin@die-bonn.de> 
*** Tel.: 							<0228 3294 278> 
*** 
*** Dateiname des Output-Files: 	<Geringqualifizierte2014.log> 
*** 
*** 
*** Grundriss des Programms: 
***			<Programm zur Erfassung der regionalen Weiterbildungsbeteiligung und der erwarteten Weiterbildungsbeteiligung von Armen> 
***
*** 
...
***				Ausbildung (nominal)
***				BA (in Berufsausbildung)
***				SB (Schulbesuch)
***				OBA (ohne Berufsausbildung)
***				GBA (geringfügige Berufsausbildung)
***				NEE Haushaltsnettoeinkommen
***				Kinder (<14)
***				Erwachsene2
***				ArmutS Armutsschwelle
***				BedarfGewicht
***
***   
***            
...
keep if EF30!=3 & EF44>=25 & EF44<=54
  
 ****Arme
 
 gen Kinder=EF683
 gen Erwachsener2 = EF616-Kinder-1 // ich gehe davon aus, dass die Gesamtzahl der Personen im Haushalt aus Plausibilitätsgründe nicht geringer ist als die Summe der Erwachsenen und Kinder + 1 
 drop if Erwachsener2<0
 
 
 gen BedarfGewicht= (Kinder*0.3)+(Erwachsener2*0.5)+1
gen ArmutS=. 
replace ArmutS= 917 * BedarfGewicht 
gen NEE=.
replace NEE= 75 if EF707==1
replace NEE= 225 if EF707==2
replace NEE= 400 if EF707==3 
replace NEE= 600 if EF707==4 
replace NEE= 800 if EF707== 5
replace NEE= 1000 if EF707== 6
replace NEE= 1200 if EF707== 7
replace NEE= 1400 if EF707== 8
replace NEE= 1600 if EF707== 9
replace NEE= 1850 if EF707== 10
replace NEE= 2150 if EF707== 11
replace NEE= 2450 if EF707== 12
replace NEE= 2750 if EF707== 13
replace NEE= 3050 if EF707== 14
replace NEE= 3400 if EF707== 15
replace NEE= 3800 if EF707== 16
replace NEE= 4250 if EF707== 17
replace NEE= 4750 if EF707== 18
replace NEE= 5250 if EF707== 19
replace NEE= 5750 if EF707== 20
replace NEE= 6750 if EF707== 21
replace NEE= 8750 if EF707== 22
replace NEE= 14000 if EF707== 23
replace NEE= 18000 if EF707== 24 
replace NEE=.  if EF707== 50
replace NEE=.  if EF707== 99

gen Arm=.
replace Arm=0 if NEE>ArmutS
replace Arm=1 if NEE<=ArmutS
replace Arm=. if NEE==.
replace Arm=. if ArmutS==.


*****erwartet in Bundesländern (gewichtet)nur für Geringqualifizierte-> das brauche ich
tab Arm [iw=EF952]
sort EF1
table EF1[iw=EF952] if Arm==1, contents(sum WBp2)

***beobachtet in Bundesländern (gewichtet)nur für Geringqualifizierte-> das brauche ich
tab EF1 WBTeilnahme [iw=EF952] if WBp2!=. & Arm==1 
 ...
 *****erwartet in Bundesländern
 tab Arm 
sort EF1
table EF1 if Arm==1, contents(sum WBp3)
 
 ***beobachtet in Bundesländern -> das brauche ich
 tab EF1 WBTeilnahme if WBp2!=. & Arm==1 
